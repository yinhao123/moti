	...
	<view class="content">
		...
		<view class="category-list">
			...
			<scroll-view scroll-y="true" class="left">
				<view class="row" v-for="(category, index) in categoryList" :key="index" :class="[index == showCategoryIndex ? 'on' : '']" @tap="switchCategory(index)">
			...
			<scroll-view
				...
				:key="categoryIndex"
				@scrolltolower="pullUpLoad(categoryIndex)"
				v-show="categoryIndex == showCategoryIndex"
			>
				<view class="category">
					...
					<view class="list">
						<view class="box" v-if="category" v-for="(box, boxIndex) in category.list" :key="boxIndex" @tap="toGoodsDetail(box.id)">
							<view class="boxImg"><image :src="box.img"></image></view>
							<view class="boxText">
...
<script>
import { addCar, queryGoodsSpuByCategroy, queryCategroyList, checkLoginName } from '@/common/request.js';
...
export default {
	...
	methods: {
		...
		async getGoodsList(index, cId, page, rows) {
			// 获取商品列表
			page = page || 1; // 默认请求第一页
			...
			if (data.code === '0') {
				...
				category.list = category.list.concat(
